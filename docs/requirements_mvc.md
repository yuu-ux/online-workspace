...
| ルーム | ルーム一覧 | 作成されたルームを一覧表示できる |
| ルーム | ルーム参加 | ユーザーが任意のルームに参加できる |
...
以下はMVPの主要フローには含めないが、現行APIでは実装済みである。
...
ログイン済みユーザーは、参加可能なすべての作業ルームに参加できる。未認証ユーザーの匿名参加や参加名だけによる参加には対応しない。
...
- 現在の参加人数を表示する
- ルーム作成日時を表示する
- ログイン済みユーザーは一覧から参加可能なルームを選択して参加できる
...
| room:member_count_changed | ルームの現在の参加人数が変化した |
| room:created | ルームが作成された |
| friend:presence_changed | フレンドのオンライン状態が変化した |
