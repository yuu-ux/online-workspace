    ...
    div([class("mx-auto max-w-3xl")], [
      ...
      div([class("space-y-4")], [
        ...
        section("2. 取得する情報", [
          ...
          "ログイン状態の維持、セキュリティ対策および障害調査のため、セッション情報、アクセス記録、認証・操作に関する監査記録を取得します。",
        ]),
        section("3. 利用目的", [
          "取得した情報は、アカウントの作成・認証、ログイン状態の維持および本人確認のために利用します。",
          "プロフィール表示、ユーザー検索、フレンド管理、ルームへの入退室およびリアルタイムチャットを提供するために利用します。",
          "不正アクセス、なりすまし、迷惑行為その他の利用規約違反を検知・防止し、サービスの安全性を維持するために利用します。",
          "障害対応、問い合わせ対応、利用状況の確認およびサービスの改善のために利用します。",
        ]),
        ...
        section("5. 第三者提供・委託", [
          ...
          "サービスの運用に必要なホスティング、データベース、監視その他の業務を外部事業者に委託する場合、委託先を適切に選定し、必要な安全管理を求めます。",
        ]),
        section("6. 安全管理と保存期間", [
          "運営者は、アクセス制御、パスワードのハッシュ化、HTTPS通信その他の合理的な安全管理措置を講じます。",
          "個人情報は、利用目的の達成に必要な期間または法令上必要な期間保管します。アカウントの退会後は、サービス提供に不要なプロフィールや関連情報を削除または利用できない状態にします。ただし、セキュリティ記録やバックアップは一定期間残る場合があります。",
        ]),
        section("7. ユーザーの権利", [
        ...
        section("8. ポリシーの変更", [
          "運営者は、法令の改正、サービス内容の変更その他の必要に応じて、本ポリシーを変更することがあります。重要な変更を行う場合は、サービス上で分かりやすくお知らせします。",
          "変更後のポリシーは、本ページに掲載した時点または別途定めた効力発生日から適用します。",
        ]),
        section("9. 問い合わせ窓口", [
          "本ポリシーに関する問い合わせは、運営者が別途指定する方法で受け付けます。提出前に、実際の運営者名および問い合わせ先をこの項目に記載します。",
        ]),
      ]),
